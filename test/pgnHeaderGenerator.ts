...
describe('header generator', () => {
  ...
  describe('generateLookupHeaderEntry', () => {
    ...
    before(() => {
      ...
      testEnumeration =
        enumerations.find((e) => e.Name === 'INDUSTRY_CODE') || enumerations[0]
    })
    ...
    it('should generate a valid C lookup entry', () => {
      ...
      testEnumeration.EnumValues.forEach((enumValue) => {
        expect(result).to.include(
          `LOOKUP(${testEnumeration.Name}, ${enumValue.Value}, "${enumValue.Name}")`
        )
      })
    })
  ...
  describe('generateBitLookupHeaderEntry', () => {
    ...
    it('should generate a valid C bitfield lookup entry', () => {
      ...
      // Check basic structure
      expect(result).to.include(
        `LOOKUP_TYPE_BITFIELD(${testBitEnumeration.Name},`
      )
      expect(result).to.include('LOOKUP_END')
      ...
      testBitEnumeration.EnumBitValues.forEach((bitValue) => {
        expect(result).to.include(
          `LOOKUP_BITFIELD(${testBitEnumeration.Name}, ${bitValue.Bit}, "${bitValue.Name}")`
        )
      })
    })
    ...
    it('should handle custom bit enumeration', () => {
      ...
      expect(result).to.include(
        'LOOKUP_BITFIELD(TEST_BIT_ENUM, 7, "Bit Seven")'
      )
      expect(result).to.include('LOOKUP_END')
    })
    ...
    it('should calculate correct bit sizing', () => {
      const customBitEnum: BitEnumeration = {
        Name: 'TEST_SIZE_ENUM',
        MaxValue: 255,
        EnumBitValues: [{ Name: 'High Bit', Bit: 15 }]
      }
  ...
  describe('generateFieldTypeLookupHeaderEntry', () => {
    ...
    it('should generate a valid C fieldtype lookup entry', () => {
      ...
      const result = generateFieldTypeLookupHeaderEntry(
        testFieldTypeEnumeration
      )

      // Check basic structure
      expect(result).to.include(
        `LOOKUP_TYPE_FIELDTYPE(${testFieldTypeEnumeration.Name},`
      )
      expect(result).to.include('LOOKUP_END')
    ...
    it('should handle custom field type enumeration', () => {
      ...
      expect(result).to.include(
        'LOOKUP_FIELDTYPE(TEST_FIELDTYPE_ENUM, 1, "Temperature", "TEMPERATURE")'
      )
      expect(result).to.include(
        'LOOKUP_FIELDTYPE(TEST_FIELDTYPE_ENUM, 2, "Pressure", "PRESSURE_UFIX16_HPA")'
      )
      expect(result).to.include('LOOKUP_END')
    })
  })
      ...
      const enums: Enumeration[] = [
        {
          Name: 'SINGLE_ENUM',
          MaxValue: 1,
          EnumValues: [{ Name: 'Only Value', Value: 0 }]
        }
      ]
